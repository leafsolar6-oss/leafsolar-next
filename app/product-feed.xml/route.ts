...
export async function GET() {
  ...
  const items = products.flatMap(product => {
    ...
    return [`<item><g:id>${xml(product.sku || product.id)}</g:id><g:title>${xml(product.name)}</g:title><g:description>${xml(product.description)}</g:description><g:link>${xml(`${base}/products/${product.slug}`)}</g:link><g:image_link>${xml(primaryImage)}</g:image_link>${additionalImages}<g:availability>${product.inStock ? 'in_stock' : 'out_of_stock'}</g:availability><g:price>${product.price.toFixed(2)} NGN</g:price><g:brand>${xml(product.brand)}</g:brand><g:product_type>${xml(`${product.department} > ${product.categoryLabel}`)}</g:product_type></item>`];
  }).join('');
