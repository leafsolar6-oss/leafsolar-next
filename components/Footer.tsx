    ...
    <footer className="bg-[#0d1711] text-gray-300">
      ...
      <div className="container-wide grid gap-10 py-12 sm:grid-cols-2 lg:grid-cols-[1.35fr_1fr_1fr_1fr_1.2fr]">
        <div>
          ...
          <p className="mt-4 max-w-sm text-sm leading-relaxed text-white/70">{site.sub}</p><div className="mt-5 flex flex-wrap gap-2 text-[9px] font-black uppercase tracking-wider text-white/70"><span className="rounded-md border border-white/10 px-2.5 py-1.5">Fouani Authorized</span><span className="rounded-md border border-white/10 px-2.5 py-1.5">{site.rcNumber}</span></div>
        </div>
        <FooterLinks title="Electronics" links={[["/home-appliances-ibadan","All home appliances"],["/products?c=Televisions","TVs"],["/products?c=Fridges%20%26%20Freezers","Fridges & freezers"],["/products?c=Air%20Conditioners","Air conditioners"],["/products?c=Washers%20%26%20Dryers","Washers & dryers"]]} />
        <FooterLinks title="Power & solar" links={[["/solar-installation-ibadan","Solar installation in Ibadan"],["/gallery","Installation gallery"],["/packages","Solar packages"],["/solar-products","Solar equipment"],["/solar-calculator","Solar calculator"],["/blog","Solar guides"]]} />
        ...
        <div><h2 className="text-sm font-bold text-white">Visit or contact us</h2><address className="mt-4 space-y-3 text-sm not-italic leading-relaxed text-white/70"><p>{site.address}</p><p><a className="font-bold text-white hover:text-leaf-300" href={`tel:${site.phoneHref}`}>{site.phone}</a></p><p><a className="hover:text-white" href={`mailto:${site.email}`}>{site.email}</a></p></address><a href={whatsappUrl()} className="btn mt-5 bg-[#25D366] px-4 py-2.5 text-xs text-emerald-950 hover:bg-[#20bd5a]">Chat on WhatsApp</a></div>
      </div>

      <div className="border-t border-white/10"><div className="container-wide flex flex-col gap-2 py-5 text-[11px] text-white/60 sm:flex-row sm:items-center sm:justify-between"><p>© {new Date().getFullYear()} Leaf Solar Ltd. All rights reserved. · <Link className="hover:text-white" href="/privacy">Privacy</Link></p><p>Secure Paystack payments · Verified order confirmation · Ibadan-based support</p></div></div>
    </footer>
  );
}

function FooterLinks({ title, links }: { title: string; links: [string, string][] }) {
  return <div><h2 className="text-sm font-bold text-white">{title}</h2><ul className="mt-4 space-y-2.5 text-sm text-white/70">{links.map(([href,label]) => <li key={href+label}><Link className="transition hover:text-white" href={href}>{label}</Link></li>)}</ul></div>;
}
